<?php
	...
	/*function QLoteDevaluado() {
		$sql = "
			...
			InvLote.M_PrecioTroquelado
			FROM InvLote
			INNER JOIN CP_QLoteArticuloDevaluado ON CP_QLoteArticuloDevaluado.InvLoteId = InvLote.Id
			ORDER BY invlote.M_PrecioTroquelado, invlote.M_PrecioCompraBruto DESC
		";
		return $sql;
	}*/
...
/**********************************************************************/
   ...
   */
	...
	/*function QG_Dias_EnCero() {
		...
		InvArticulo.CodigoArticulo AS CodigoInterno,
		InvArticulo.Descripcion,
		(ROUND(CAST(SUM (InvLoteAlmacen.Existencia) AS DECIMAL(38,0)),2,0)) as Existencia,
		InvArticulo.FinConceptoImptoIdCompra AS ConceptoImpuesto
		FROM InvArticulo
		INNER JOIN InvLoteAlmacen ON InvArticulo.Id=InvLoteAlmacen.InvArticuloId
		WHERE InvLoteAlmacen.Existencia > 0
  ...
  function QG_Integracion_ProductoCaida($RangoDias) {
	...
	/*
    TITULO: QG_DetalleArticulo_ProductoCaida
    PARAMETROS: [$IdArticulo] $IdArticulo del articulo a buscar
    FUNCION: Query que genera el detalle del articulo solicitado
    RETORNO: Detalle del articulo
   	*/
  	function QG_DetalleArticulo_ProductoCaida($IdArticulo) {
	    $sql = " 
	      SELECT
	      InvArticulo.Id,
	      InvArticulo.CodigoArticulo,
