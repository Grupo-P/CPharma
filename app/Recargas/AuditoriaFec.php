...
class AuditoriaFec extends Model
{
    ...
    public static function totalDivisas($cajero,$fini,$ffin){
        $operaciones = OperacionFec::whereBetween('fecha',[ $fini,$ffin])->where('id_user','=',$cajero)->get();
        $resultado = 0;
        foreach($operaciones as $operacion){
            $auditoria = AuditoriaFec::where('id_operacion','=',$operacion->id)->first();
            if($auditoria != null){
    ...
    public static function totalDivisasCaja($caja,$fini,$ffin){
        $operaciones = OperacionFec::whereBetween('fecha',[ $fini,$ffin])->where('id_caja','=',$caja)->get();
        $resultado = 0;
        foreach($operaciones as $operacion){
            $auditoria = AuditoriaFec::where('id_operacion','=',$operacion->id)->first();
            if($auditoria != null){
    ...
    public static function cantidadRecargasCajero($fini,$ffin,$cajero){        
        $cantidad = AuditoriaFec::whereBetween('fecha',[ $fini,$ffin])->where('id_usuario','=',$cajero)->count();        
        return $cantidad;
    }

    public static function cantidadRecargasCaja($fini,$ffin,$caja){        
        $cantidad = AuditoriaFec::whereBetween('fecha',[ $fini,$ffin])->where('id_caja','=',$caja)->count();        
        return $cantidad;
    }
}
