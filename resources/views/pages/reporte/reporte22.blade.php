    ...
    echo'Tiempo de carga: '.$IntervalCarga->format("%Y-%M-%D %H:%I:%S");
        ...
        <table style="width:100%;">
          <tr>
            <td style="width:90%;">
        ';
        echo'<select name="atributo" class="form-control" required="required">';
        echo'<option value="">Seleccione una opción...</option>';
          while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
            echo'<option value="'
            .$row['Descripcion'].'">'
            .FG_Limpiar_Texto($row['Descripcion']).'</option>';
          }
        echo'</select>';
        echo'
            </td>
...
<?php
 /************************************************************/
 /*
    TITULO: Q22_Lista_Atributos
    FUNCION: arma la lista de atributos de la sede
    RETORNO: No aplica
    DESAROLLADO POR: SERGIO COVA
  */
 function Q22_Lista_Atributos(){
  $sql = "
    SELECT
    InvAtributo.Id AS IdAtributo,
    InvAtributo.Descripcion AS Descripcion
    FROM InvAtributo
  ";
  return $sql;
 }
 /************************************************************/
 /*
    TITULO: Q22_Atributos_Descrip
    FUNCION: arma la lista de atributos de la sede
    RETORNO: No aplica
    DESAROLLADO POR: SERGIO COVA
  */
 function Q22_Atributos_Descrip($Descripcion){
  $sql = "
    SELECT
    InvAtributo.Id AS IdAtributo,
    InvAtributo.Descripcion AS Descripcion
    FROM InvAtributo
    WHERE InvAtributo.Descripcion like '$Descripcion'
  ";
  return $sql;
 }
 /************************************************************/
 /*
    TITULO: Q22_Articulos_Atributo
    FUNCION: arma la lista de atributos de la sede
    RETORNO: No aplica
    DESAROLLADO POR: SERGIO COVA
  */
 function Q22_Articulos_Atributo($IdAtributo){
  $sql = "
    SELECT *
 ...
 /************************************************************/
 /*
    TITULO: R22_Articulos_Atributo
    FUNCION: arma la lista de atributos de la sede
    RETORNO: No aplica
    DESAROLLADO POR: SERGIO COVA
  */
 function R22_Articulos_Atributo($SedeConnection,$Atributo){
    $conn = FG_Conectar_Smartpharma($SedeConnection);
    $sql = Q22_Atributos_Descrip($Atributo);
    $result = sqlsrv_query($conn,$sql);
    $row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC);
    $IdAtributo = $row['IdAtributo'];

    $sql1 = Q22_Articulos_Atributo($IdAtributo);
    ...
    while($row1 = sqlsrv_fetch_array($result1, SQLSRV_FETCH_ASSOC)) {
      ...
      $sql2 = R22Q_Detalle_Articulo($IdArticulo);
 ...
 /**********************************************************************************/
  /*
    TITULO: R22Q_Detalle_Articulo
    FUNCION: Query que genera el detalle del articulo solicitado
    RETORNO: Detalle del articulo
    DESAROLLADO POR: SERGIO COVA
  */
  function R22Q_Detalle_Articulo($IdArticulo) {
    $sql = "
      SELECT
--Id Articulo
