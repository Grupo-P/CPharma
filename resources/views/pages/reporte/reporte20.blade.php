  ...
  //CASO 2: CARGA AL HABER SELECCIONADO UN PROVEEDOR
    ...
    if(FG_Validar_Conectividad('FTN')==1){
      ...
      if(FG_Validar_Conectividad('FSM')==1){
        ...
        R20_Catalogo_Proveedor_C2($_GET['SEDE'],'FSM',$_GET['Id'],$_GET['Nombre'],$_GET['fechaInicio'],$_GET['fechaFin'],$_GET['pedido']);
        FG_Guardar_Auditoria('CONSULTAR','REPORTE','Tri Tienda Por Proveedor');
      }
      else{
      ...
      if(FG_Validar_Conectividad('FEC')==1){
        ...
        R20_Catalogo_Proveedor_C2($_GET['SEDE'],'FEC',$_GET['Id'],$_GET['Nombre'],$_GET['fechaInicio'],$_GET['fechaFin'],$_GET['pedido']);
        FG_Guardar_Auditoria('CONSULTAR','REPORTE','Tri Tienda Por Proveedor');
      }
      else{
      ...
      if(FG_Validar_Conectividad('FLF')==1){
        ...
        R20_Catalogo_Proveedor_C2($_GET['SEDE'],'FLF',$_GET['Id'],$_GET['Nombre'],$_GET['fechaInicio'],$_GET['fechaFin'],$_GET['pedido']);
        FG_Guardar_Auditoria('CONSULTAR','REPORTE','Tri Tienda Por Proveedor');
      }
      else{
...
<?php
  /**********************************************************************************/
  /*
    TITULO: R20_Catalogo_Proveedor_C2
    FUNCION: Armar el reporte catalogo de proveedor
    RETORNO: No aplica
    DESAROLLADO POR: SERGIO COVA
  */
  function R20_Catalogo_Proveedor_C2($SedeConnectionOrigen,$SedeConnectionRemota,$IdProveedor,$NombreProveedor,$FInicial,$FFinal,$DiasPedido){

    $connRemoto = FG_Conectar_Smartpharma($SedeConnectionRemota);
    $connOrigen = FG_Conectar_Smartpharma($SedeConnectionOrigen);
    $sql = R20Q_Catalogo_Proveedor($IdProveedor);
    $result = sqlsrv_query($connOrigen,$sql);
      ...
      <table class="table table-striped table-bordered col-12 sortable" id="myTable">
          <thead class="thead-dark">
            <tr>
              ...
              <th scope="col" class="CP-sticky">Ultimo Precio</br>(Sin IVA) '.SigVe.'</th>
              <th scope="col" class="CP-sticky">Ultima Venta</th>
              <th scope="col" class="CP-sticky">Tiempo Tienda</br>(Dias)</th>
            </tr>
          </thead>
          <tbody>
      ';
    ...
    while($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
      $CodigoBarraOrigen = $row['CodigoBarra'];

      $sql1 = R20Q_Detalle_Articulo_Cod($CodigoBarraOrigen);
      $result1 = sqlsrv_query($connRemoto,$sql1);
      ...
      $UltimoPrecio = $row1["UltimoPrecio"];
      $TiempoTienda = $row1["TiempoTienda"];
      /*
      $sql2 = R20Q_Integracion_Pedido_Productos($IdArticulo,$FInicial,$FFinal);
      $result2 = sqlsrv_query($connRemoto,$sql2);
      $row2 = sqlsrv_fetch_array($result2, SQLSRV_FETCH_ASSOC);
      $UnidadesVendidas = intval($row2["TotalUnidadesVendidas"]);
      ...
      */
        ...
        if($CodigoBarra!=NULL&&$CodigoBarra!=''){
          ...
          <a href="/reporte2?Id='.$IdArticulo.'&SEDE='.$SedeConnectionOrigen.'" style="text-decoration: none; color: black;" target="_blank">'
            .$Descripcion.
          '</a>
          </td>';
          ...
          if($row1["UltimaVenta"]!=NULL){
            echo '<td align="center">'.$row1["UltimaVenta"]->format('d-m-Y').'</td>';
          }
          else{
      ...
      echo '</tbody></table>';
    sqlsrv_close($connOrigen);
    sqlsrv_close($connRemoto);
  }
  /**********************************************************************************/
  /*
    TITULO: R20Q_Lista_Proveedores
    FUNCION: Armar una lista de proveedores
    RETORNO: Lista de proveedores
    DESAROLLADO POR: SERGIO COVA
  */
  function R20Q_Lista_Proveedores() {
    $sql = "
      SELECT
  ...
  /**********************************************************************************/
  /*
    TITULO: R20Q_Catalogo_Proveedor
    FUNCION: Armar una lista de proveedores
    RETORNO: Lista de proveedores
    DESAROLLADO POR: SERGIO COVA
  */
  function R20Q_Catalogo_Proveedor($IdProveedor) {
    $sql = "
      SELECT
      InvArticulo.Id,
  ...
  /**********************************************************************************/
  /*
    TITULO: R20Q_Detalle_Articulo_Cod
    FUNCION: Query que genera el detalle del articulo solicitado
    RETORNO: Detalle del articulo
    DESAROLLADO POR: SERGIO COVA
  */
  function R20Q_Detalle_Articulo_Cod($CodigoBarra) {
    $sql = "
      SELECT
--Id Articulo
...
--Ordanamiento
  ...
  /**********************************************************************************/
  /*
    TITULO: R20Q_Integracion_Pedido_Productos
    FUNCION: Ubicar el top de productos mas vendidos
    RETORNO: Lista de productos mas vendidos
    DESAROLLADO POR: SERGIO COVA
  */
  function R20Q_Integracion_Pedido_Productos($IdArticulo,$FInicial,$FFinal) {
    $sql = "
      SELECT
    -- Id Articulo
