...
@section('content')
  ...
  <h1 class="h5 text-info">
    ...
    Registro de pagos en efectivo bolívares GP
  </h1>
  ...
  <table style="width:100%;" class="CP-stickyBar">
    <tr>
      ...
      @if(auth()->user()->departamento == 'GERENCIA' || auth()->user()->departamento == 'TESORERIA' || auth()->user()->departamento == 'TECNOLOGIA' || auth()->user()->departamento == 'ADMINISTRACION')
        ...
        <td style="width:20%;" align="center">
          <a href="/bolivaresGP/create?tipo=proveedores" role="button" class="btn btn-outline-info btn-sm"
          style="display: inline; text-align: left;">
          <i class="fa fa-plus"></i>
  ...
  <table class="table table-striped table-borderless col-12" id="myTable">
    ...
    @foreach($pagos as $pago)
      <tr>
        ...
        <td class="plan_cuentas">{{isset($pago->cuenta->nombre) ? $pago->cuenta->nombre : ''}}</td>
        ...
        <td style="width:140px;">
            @if(!$pago->ingresos)
                @if (!($pago->egresos && strpos($pago->concepto, 'DIFERIDO')))
                    <a target="_blank" href="/bolivaresGP/soporte/{{$pago->id}}" role="button" class="btn btn-outline-success btn-sm" data-toggle="tooltip" data-placement="top" title="Ver soporte">
