...
@section('title', 'Registro de pagos en efectivo bolívares FLF')
...
@section('content')
  ...
  <h1 class="h5 text-info">
    ...
    @if($request->get('tipo') == 'movimiento')
      Agregar movimiento bolívares FLF
    @endif

    @if($request->get('tipo') == 'proveedores')
      Agregar pago a proveedores bolívares FLF
    @endif
  </h1>
  <hr class="row align-items-start col-12">

  <a href="/bolivaresFLF" class="btn btn-outline-info btn-sm">
  ...
  {!! Form::open(['route' => 'bolivaresFLF.store', 'method' => 'POST', 'id' => 'crear_movimientos', 'class' => 'form-group']) !!}
    @if($request->get('tipo') == 'movimiento')
      <fieldset>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        ...
                        <input readonly class="form-control" name="moneda" type="text" type="text" value="{{ ($prepagado != '') ? $prepagado->proveedor->moneda : '' }}">
                    </td>

                    <td>
                        <label for="saldo">Saldo</label>
                        <input readonly class="form-control" name="saldo" type="text" type="text" value="{{ ($prepagado != '') ? $prepagado->proveedor->saldo : '' }}">
                    </td>

                    <td>
                        <label for="saldo_iva">IVA</label>
                        <input readonly class="form-control" name="saldo_iva" type="text" type="text" value="{{ ($prepagado != '') ? $prepagado->proveedor->saldo_iva : '' }}">
                    </td>

                     <td>
                        <label for="moneda_iva">Moneda IVA</label>
                        <input readonly class="form-control" name="moneda_iva" type="text" type="text" value="{{ ($prepagado != '') ? $prepagado->proveedor->moneda_iva : '' }}">
                    </td>
                </tr>
            </tbody>
        </table>
      ...
      <script>
          $(document).ready(function() {
              ...
              function activar_tasa_iva() {
                  ...
                  if (moneda_iva != 'Bolívares') {
                      $.ajax({
                          method: 'POST',
                          url: '/bolivaresFLF/validar',
                          data: {
                              moneda_iva: $('[name=moneda_iva]').val(),
                              _token: '{{ csrf_token() }}'
                          },
